<template>
    <li>
        <div class="sidebar-group">
            <DropDownBtn class="sidebar-group__title" @click="switchVisibility()" :visible="visible"> {{ group.name }} </DropDownBtn>
        ...
        <ul v-show="visible">
            <li class="sidebar-item" v-for="channel in group.listChannel" :key="channel.id">
                ...
                <ButtonPopUp class="config-btn" :label="'Settings'" v-if="user.role_id == 1" @click="OPEN_CONFIG({...channel, channelGroupId : group.id })">
...
export default {
    components: { ButtonPopUp, DropDownBtn },
    data() {
        return {
            visible: true,
        };
