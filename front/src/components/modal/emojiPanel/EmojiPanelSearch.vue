    ...
    <form class="form-emoji">
        <div class="form-emoji__search-bar">
            <div class="form-emoji__field" :class="{ 'form-emoji__field--active': filterValue }">
                ...
                <div class="form-emoji__btn" v-show="!filterValue">
                ...
                <button type="button" class="form-emoji__btn" aria-label="Annuler" v-show="filterValue" @click="deleteValue">
                    <svg aria-hidden="false" width="20" height="20" viewBox="0 0 24 24">
                        <path
                            fill="currentColor"
            ...
            <div class="select-skin" :class="{'select-skin--active' : panelSkinShow }" >
                <div class="select-skin__list-item">
                    <img :src="require('@/assets/twemoji/svg/' + listSkinUnicode[selectedSkin] + '.svg')" :alt="`Skin selectionnée : ${selectedSkin}`" @click="switchPanelSkin()">
                </div>
                <ul class='select-skin__list' v-show="panelSkinShow">
                    <li class='select-skin__list-item' v-for="(skin,index) in listSkinUnicode" :key="skin" @click="updateSelectedSkin(index)">
                        <img :src="require('@/assets/twemoji/svg/' + skin + '.svg')" :alt="`Skin ${skin}`"  >
                    </li>
                </ul>
            </div>
