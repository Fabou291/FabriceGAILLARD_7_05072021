<template>
    <li>
        <div class="sidebar-group">
            <BtnDropDown class="sidebar-group__title" @click="switchVisibility()" :visible="visible"> {{ group.name }} </BtnDropDown>
            <BtnPopUp v-if="user.role_id == 1" :label="'Ajouter un channel'" @click="OPEN( { id : group.id, name : group.name } )">
                <svg aria-hidden="false" width="18" height="18" viewBox="0 0 18 18">
                    <polygon
                        fill-rule="nonzero"
                        fill="currentColor"
        ...
        <ul v-show="visible">
            <li class="sidebar-item" :class="{ 'sidebar-item--active' : $route.params.id == channel.id }" v-for="channel in group.listChannel" :key="channel.id">
                <router-link class="sidebar-item__link" :to="`/channel/${channel.id}`" @click="SET_VISIBILITY(false)">
                    ...
                    <span class="sidebar-item__link-name" >{{ channel.name }}</span>
                </router-link>

                <BtnPopUp class="config-btn" :label="'Settings'" v-if="user.role_id == 1" @click="OPEN_CONFIG({...channel, channelGroupId : group.id })">
...
import BtnDropDown from "@/components/btn/BtnDropDown.vue";
import BtnPopUp from "@/components/btn/BtnPopUp.vue";



export default {
    components: { BtnPopUp, BtnDropDown },
    data() {
        return {
            visible: true,
        };
